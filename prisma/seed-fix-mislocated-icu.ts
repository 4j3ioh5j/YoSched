 ...
 * The column-shift import had placed ICU on the WRONG staff on these days;
...
// [date, staff currently-wrongly-ICU, printed main-grid code]
...
async function main() {
  ...
  for (const [dateStr, initials, code] of FIX) {
    ...
    if (!staffId || !shiftTypeId) { console.warn(`SKIP ${dateStr} ${initials}: lookup failed`); skipped++; continue; }
