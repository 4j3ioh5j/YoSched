...
function makeStaff(id: string, initials: string, fte = 1) {
...
describe("computeFairness", () => {
  ...
  it("skips inactive staff", () => {
    const result = computeFairness({
      assignments: [],
      staff: [{ ...makeStaff("p1", "AB"), isActive: false }],
      desirabilityWeights: [],
      holidays: [],
    });
  ...
  it("skips non-auto-scheduled staff", () => {
    const result = computeFairness({
      assignments: [],
      staff: [{ ...makeStaff("p1", "AB"), isAutoScheduled: false }],
      desirabilityWeights: [],
      holidays: [],
    });
    expect(result.metrics).toHaveLength(0);
  ...
  describe("multi-staff averages", () => {
    it("computes correct averages across staff", () => {
      const result = computeFairness({
        assignments: [
          makeAssignment("p1", "2025-05-12", "OR", "st1"),
          makeAssignment("p1", "2025-05-13", "OR", "st1"),
