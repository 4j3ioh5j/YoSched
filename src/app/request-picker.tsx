...
type Props = {
  ...
  targetCount: number; // providers covered by the selection (for the label)
...
function markGlyph(m: { polarity: "accept" | "negate"; strength: RequestStrength }) {
  const sign = m.polarity === "negate" ? "✗" : "○";
  return { sign, faint: m.strength === "soft" };
}

export function RequestPicker({ shiftTypes, position, targetCount, onSave, onClose }: Props) {
  const ref = useRef<HTMLDivElement>(null);
  // Work-shift marks keyed by shiftTypeId; OFF and leave tracked separately.
  ...
  useEffect(() => {
    ...
    if (rect.right > window.innerWidth) ref.current.style.left = `${position.x - rect.width - 12}px`;
    if (rect.bottom > window.innerHeight) ref.current.style.top = `${position.y - rect.height - 12}px`;
  }, [position]);
  ...
  function toggleShift(id: string, e: React.MouseEvent) {
    ...
    setShiftMarks((prev) => {
      ...
      const cur = m.get(id);
      // Same exact mark → clear it (toggle off); otherwise set/replace.
      if (cur && cur.polarity === next.polarity && cur.strength === next.strength) {
        m.delete(id);
      } else {
        m.set(id, { shiftTypeId: id, ...next });
      }
      return m;
    });
  }
  ...
  function save() {
    ...
    onSave({
      shiftMarks: [...shiftMarks.values()],
      offStrength,
      leaveShiftTypeIds: [...leaveIds],
    });
  }
  ...
  return (
    <div
      ref={ref}
      className="fixed z-50 bg-slate-800 border border-violet-500/60 rounded-lg shadow-xl p-2 min-w-[230px] max-h-[440px] overflow-y-auto"
      style={{ left: position.x + 12, top: position.y + 12 }}
    >
      <div className="text-[10px] font-semibold text-violet-300 px-1 pb-1 border-b border-slate-700 mb-1">
        New request{targetCount > 1 ? ` · ${targetCount} providers` : ""}
      </div>

      <div className="text-[10px] uppercase tracking-wider text-slate-500 px-1 py-0.5">Work</div>
      ...
      {leaveShifts.length > 0 && (
        <>
          <div className="text-[10px] uppercase tracking-wider text-slate-500 px-1 py-0.5 mt-1">Leave</div>
          <div className="grid grid-cols-3 gap-0.5">
            {leaveShifts.map((st) => (
              <button
                key={st.id}
      ...
      {offShift && (
        <div className="border-t border-slate-700 mt-2 pt-1">
          <button
            ...
            className={[
              "w-full px-2 py-1.5 text-xs font-bold rounded text-center text-slate-300 bg-slate-700/50 hover:bg-slate-600/50 transition-colors",
      ...
      <div className="text-[9px] text-slate-500 px-1 pt-1.5 leading-snug">
      ...
      <div className="border-t border-slate-700 mt-1 pt-1">
        <button
          ...
          className="w-full px-2 py-1.5 text-xs font-semibold rounded transition-colors bg-violet-600/80 hover:bg-violet-600 text-white disabled:opacity-40 disabled:cursor-not-allowed"
        >
